...
const changeAdminAction = async(provider:NetworkProvider, ui:UIProvider) => {
    ...
    // Check if we can access the contract before changing admin
    try {
        await minterContract.getAdminAddress();
    } catch (error) {
        throw new Error("Contract is not active or accessible");
    }
...
const mintAction = async (provider:NetworkProvider, ui:UIProvider) => {
    ...
    // Check if contract is accessible before minting
    try {
        await minterContract.getJettonData();
    } catch (error) {
        throw new Error("Contract is not active or accessible");
    }
...
export async function run(provider: NetworkProvider) {
    ...
    do {
        ...
        // Test contract accessibility by trying to get jetton data
        try {
            const testContract = provider.open(JettonMinter.createFromAddress(minterAddress));
            const jettonData = await testContract.getJettonData();
            
            // Check if contract code matches expected
            const minterCode = await compile('JettonMinter');
            // Skip code verification for simplicity, just check if we can call methods
